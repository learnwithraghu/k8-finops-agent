# Section 08 Guide: LLM Decision Flow
...
- `sections/08-llm-agent-langchain/agent/`
...
```bash
cat sections/08-llm-agent-langchain/agent/analyzer.py
```
...
## Step 6: Install the Section 08 dependencies
```bash
pip install -r sections/08-llm-agent-langchain/requirements.txt
```
...
```bash
PYTHONPATH=sections/08-llm-agent-langchain python3 -m agent.main
```
...
- `sections/09-issue-tracker-service/guide.md`

Section 09 will introduce the service that stores or routes the decision output.

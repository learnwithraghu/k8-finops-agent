# Section 10 Guide: Helm-Based Deployment

This is the only file you need for Section 10.

## Goal
Package the Section 07 LLM structured agent as a Helm chart and deploy it inside Kubernetes as a scheduled CronJob. Instead of managing raw Kubernetes manifests, use Helm for templating, versioning, and lifecycle management, so the FinOps scan runs in-cluster with professional packaging.

## Tutor note
Section 10 is not about rebuilding the agent logic. The code follows the Sections 06 and 07 MCP-agent track (full in-cluster MCP wiring is follow-up work). The learning focus is:
...
Complete Sections 01 through 09 first.

You should already have:
- a working Kind cluster
- the airline app deployed (from Section 01)
- Section 07 LLM structured agent running locally against the cluster
- a working `.env` file with your OpenAI-compatible API key
- Helm 3.x installed (`helm version`)
- Docker installed for building images

## Where the files live
- `sections/10-k8-native-agent/agent/` — same agent code as Sections 06 and 07 (transitional; rewrite in follow-up session)
- `sections/10-k8-native-agent/config/` — reference tagging policy
- `sections/10-k8-native-agent/docker/Dockerfile` — container build
- `sections/10-k8-native-agent/helm/` — **NEW: Helm chart**
...
## Step 1: Confirm the local agent still works

Before containerizing, verify the local agent pipeline still works from your laptop (Section 06 collector + Section 07 LLM structured agent):
...
- `taggingRules` section embeds the entire tagging policy from Section 07

Compare the embedded tagging rules with the original:

```bash
# View just the tagging rules from values.yaml
yq '.taggingRules' sections/10-k8-native-agent/helm/values.yaml

# Compare with the original from Section 07
...
cat sections/10-k8-native-agent/manifests/namespace.yaml

# Templated namespace
cat sections/10-k8-native-agent/helm/templates/namespace.yaml
```
...
```bash
helm template finops-agent sections/10-k8-native-agent/helm/ > /tmp/rendered-manifests.yaml
...
```bash
helm template finops-agent sections/10-k8-native-agent/helm/ | kubectl apply --dry-run=client -f -
```
...
- The report matches what you saw in Section 07
...
helm upgrade finops-agent sections/10-k8-native-agent/helm/ --reuse-values
```
...
```bash
helm upgrade finops-agent sections/10-k8-native-agent/helm/ -f /tmp/custom-values.yaml
```
...
```bash
helm upgrade finops-agent sections/10-k8-native-agent/helm/ --set targetNamespace="booking-api"
```
...
```bash
helm upgrade finops-agent sections/10-k8-native-agent/helm/ --set targetNamespace=""
```
...
```bash
helm upgrade finops-agent sections/10-k8-native-agent/helm/ \
...
- **Agent code unchanged:** The agent from the MCP-agent track (06 / 07) runs in-cluster
...
- Integrating with the issue tracker service from Sections 08/09

...
## Goal
Take the LLM decisions from Section 05 and turn the actionable ones into tickets in the Section 06 issue tracker service.
...
- how the Section 05 decision output maps to a tracker ticket payload
- how a tiny HTTP client can POST issues to the local tracker
...
- the Section 05 LLM agent running locally
...
- the same OpenAI-compatible settings from Section 05 are present
...
```bash
cat sections/07-agent-to-tracker-integration/agent/tracker.py
```
...
What to look for:
- the same scan + LLM flow as Section 05, always scanning all namespaces
...
```bash
PYTHONPATH=sections/07-agent-to-tracker-integration python3 -m agent.main
```
...
## What to notice
- Section 05 decides *what* to do.
- Section 07 does one new thing: it turns those decisions into tracker tickets.

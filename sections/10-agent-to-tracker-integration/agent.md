# agent.md — Section 10 Maintenance Rules
...
Section 10 teaches learners how to connect the **Section 08 LLM decision-flow agent**
to the **Section 09 issue tracker service**. The only new responsibility in this
...
| Item | Current value |
|---|---|
| LLM client library | `langchain-openai` (`ChatOpenAI`) (reused from Section 08) |
...
| Run command | `PYTHONPATH=sections/10-agent-to-tracker-integration python3 -m agent.main` |
...
- [ ] `config/tagging-rules.yaml` — if changed, confirm it stays identical to Section 08

# Section 08 Guide: Helm-Based Deployment

This is the only file you need for Section 08.

## Goal
Package the Section 05 LLM-powered FinOps agent as a Helm chart and deploy it inside Kubernetes as a scheduled CronJob. Instead of managing raw Kubernetes manifests, use Helm for templating, versioning, and lifecycle management, so the FinOps scan runs in-cluster with professional packaging.

## Tutor note
Section 08 is not about rebuilding the agent logic. The code is the same as Section 05. The learning focus is:
...
- `sections/08-k8-native-agent/config/` — reference tagging policy
- `sections/08-k8-native-agent/docker/Dockerfile` — container build
- `sections/08-k8-native-agent/helm/` — **NEW: Helm chart**
...
```bash
tree sections/08-k8-native-agent/helm/
```
...
```bash
cat sections/08-k8-native-agent/helm/Chart.yaml
```
...
```bash
cat sections/08-k8-native-agent/helm/values.yaml
```
...
- `taggingRules` section embeds the entire tagging policy from Section 05

Compare the embedded tagging rules with the original:

```bash
# View just the tagging rules from values.yaml
yq '.taggingRules' sections/08-k8-native-agent/helm/values.yaml

# Compare with the original from Section 05
...
cat sections/08-k8-native-agent/manifests/namespace.yaml

# Templated namespace
cat sections/08-k8-native-agent/helm/templates/namespace.yaml
```
...
```bash
helm template finops-agent sections/08-k8-native-agent/helm/
```
...
```bash
helm template finops-agent sections/08-k8-native-agent/helm/ > /tmp/rendered-manifests.yaml
...
```bash
helm lint sections/08-k8-native-agent/helm/
```
...
```bash
helm template finops-agent sections/08-k8-native-agent/helm/ | kubectl apply --dry-run=client -f -
```
...
```bash
helm install finops-agent sections/08-k8-native-agent/helm/
```
...
- The report matches what you saw in Section 05
...
```bash
helm upgrade finops-agent sections/08-k8-native-agent/helm/ -f /tmp/custom-values.yaml
```
...
```bash
helm upgrade finops-agent sections/08-k8-native-agent/helm/ --set targetNamespace="booking-api"
```
...
```bash
helm upgrade finops-agent sections/08-k8-native-agent/helm/ --set targetNamespace=""
```
...
```bash
helm upgrade finops-agent sections/08-k8-native-agent/helm/ \
...
- **Agent code unchanged:** The same agent from Section 05 runs in-cluster

...
# Create the baseline namespaces used by later sections
create_namespaces() {
    print_header "Step 6: Creating Baseline Namespaces"
    ...
    echo ""
    kubectl get namespaces | grep -E "^(NAME|${NAMESPACES[0]}|${NAMESPACES[1]}|${NAMESPACES[2]})"
}

# Check for existing Docker image
check_existing_image() {
    print_header "Step 7: Checking for Existing Docker Image"
...
check_env_file() {
    print_header "Step 8: Checking Environment Configuration"
    ...
    if [ -f ".env" ]; then
        ...
        if [ ${#missing_vars[@]} -ne 0 ]; then
            ...
            echo "  bash sections/01-cluster-foundation/commands/setup.sh --mock"
        fi
    else
...
print_next_steps() {
    ...
    echo -e "   ${YELLOW}kubectl apply -k sections/02-airline-app-deployment/manifests/airline-k8-deployment/${NC}"
    echo ""
    echo "2. Verify namespaces:"
    echo -e "   ${YELLOW}kubectl get namespaces${NC}"
    echo ""
    echo "3. Verify deployments:"
    echo -e "   ${YELLOW}kubectl get all -n airline${NC}"
    echo ""
    echo "4. Run the FinOps agent:"
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            --help|-h)
                ...
                echo ""
                echo "Examples:"
                echo "  bash sections/01-cluster-foundation/commands/setup.sh                    # Full setup"
                echo "  bash sections/01-cluster-foundation/commands/setup.sh --mock             # Setup for mock mode"
                echo "  bash sections/01-cluster-foundation/commands/setup.sh --cleanup          # Clean everything"
                exit 0
                ;;
            *)
